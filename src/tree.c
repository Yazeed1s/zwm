...
int render_tree(node_t *current_node, wm_t *wm) {
	...
	render_tree(current_node->first_child, wm);
	render_tree(current_node->second_child, wm);
...
void insert_node(node_t *current_node, client_t *new_client) {
	if (current_node == NULL) {
	...
	if (!has_first_child(current_node)) {
		log_message(ERROR, "current_node no fs");
		...
		log_message(ERROR, "current_node no fs done");
	}
	if (!has_second_child(current_node)) {

		log_message(ERROR, "current_node no ss");
		...
		rectangle_t r					   = {0};
		if (current_node->rectangle.width >= current_node->rectangle.height) {
			r.x		 = (int16_t)(current_node->rectangle.x +
							 current_node->rectangle.width / 2 + W_GAP);
			r.y		 = current_node->rectangle.y;
		...
		log_message(ERROR, "current_node no ss done");
	}
}
...
void resize_subtree(node_t *parent) {
	...
	if (parent->rectangle.width >= parent->rectangle.height) {
		r2.x	 = (int16_t)(parent->rectangle.x + parent->rectangle.width / 2 +
						 W_GAP);
		...
		r2.y	  = (int16_t)(parent->rectangle.height / 2 + W_GAP +
						  parent->rectangle.y);
...
void switch_node_to_desktop(node_t *node, desktop_t *nd, desktop_t *od,
							wm_t *wm) {
	if (node == NULL || od == NULL || nd == NULL || wm == NULL) {
		return;
	}
	...
	if (is_tree_empty(nd->tree)) {
		...
		nd->tree			= init_root();
		nd->tree->rectangle = r;
		nd->tree->client	= create_client(
			   node->client->window, XCB_ATOM_WINDOW, wm->connection);
	} else {
		// get leaf of nd's tree
		log_message(DEBUG, "tree %d is not empty", nd->id);
		node_t *leaf = find_left_leaf(nd->tree);
		log_message(DEBUG, "leaf is not null");
		if (leaf == NULL) {
			log_message(DEBUG, "leaf is null");
			return;
		}
		client_t *client = create_client(
			node->client->window, XCB_ATOM_WINDOW, wm->connection);
		if (leaf->client == NULL) {
			log_message(DEBUG, "leaf client is null");
			return;
		}
		log_message(INFO, "leaf = %d", leaf->client->window);
		insert_node(leaf, client);
	}
	delete_node(node, od);
}
