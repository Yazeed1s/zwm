...
int close_or_kill(xcb_window_t win, wm_t *wm) {
	...
	log_message(ERROR,
				"window %d name %s does not support wm_delete\n",
				win,
				t_reply.name);
...
void shift_cmd(xcb_key_press_event_t *key_event, wm_t *wm) {
	...
	for (int i = 1; i <= wm->n_of_desktops; ++i) {
		if (k == XK_0 + i) {
			...
			xcb_unmap_window(wm->connection, node->client->window);
			switch_node_to_desktop(node, nd, od, wm);
...
void handle_key_press(xcb_key_press_event_t *key_press, wm_t *wm) {
	...
	} else if ((key_press->state & SHIFT_MASK) != 0) {
